...
/*
  A shared token in a cookie, checked on every /admin request.

  This is not real authentication and the README says so plainly. There are no accounts, no
  per-person audit trail, and one token for everybody, so it cannot tell you who changed a lead's
...
export function middleware(request: NextRequest) {
  ...
  if (!expected) {
    return new NextResponse("The admin area is not configured. Set ADMIN_TOKEN.", {
      status: 503,
